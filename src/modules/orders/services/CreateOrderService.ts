...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    const findProductsWithNonQuantityAvaliable = products.filter(
      product =>
        existentProducts.filter(p => p.id === product.id)[0].quantity <
        product.quantity,
    );

    if (findProductsWithNonQuantityAvaliable.length) {
      throw new AppError(
        `Could not find any product ${findProductsWithNonQuantityAvaliable[0].quantity} is not avaliable ${findProductsWithNonQuantityAvaliable[0].id} `,
      );
    }
